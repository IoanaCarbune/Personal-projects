...
public class SecondModuleController {
	...
	private void initialize() throws SQLException {
		...
		UMComboBox.setItems(app.getAllMedicalUnitIds());

		logInCNP = app.getCNPUser(Main.getUserName());
		initializeSalary();
		...
		String currentFunction = app.getFunctionUser(logInCNP);
		String currentUserType = app.getTypeUser(Main.getUserName());
	...
	private void setSpecialitiesComboBox(String CNP) throws SQLException {
		...
		if (app.getFunctionUser(CNP).equals("Medic")) {
			ObservableList<String> specialitiesList = FXCollections.observableArrayList();
			ResultSet rst = app.getMedicSpecialitiesNames(CNP);
			while (rst.next()) {
	...
	private void setTotalProfit() throws SQLException {
		...
		if (app.getFunctionUser(currentCNP).equals("Medic") && !monthsMedicProfit.getSelectionModel().isEmpty()
			...
			if (year <= LocalDate.now().getYear()) {
				...
				if (year == LocalDate.now().getYear() && month >= LocalDate.now().getMonthValue())
					ok = 0;
				if (app.getSalary(currentCNP, year, month).equals("Date inexistente"))
					ok = 0;
				...
				if (ok == 1) {
					...
					double income = 0;
					double commission = 0;
					double salary = new Double(app.getSalary(currentCNP, year, month));
					ResultSet rst = app.getBillSpecificDate(year, month);
					while (rst.next()) {
						income += rst.getDouble(3);
					}

					List<ResultSet> data = app.getAllUserData(currentCNP, "Medic");
	...
	private void setClinicProfit() throws SQLException {
		...
		if (!monthsClinicProfitComboBox.getSelectionModel().isEmpty()
			...
			ResultSet rstCNP = app.getAllUsersCNP();
			double total = 0;
			double medicTotal = 0;
			...
			while (rstCNP.next()) {
				...
				if (!app.getSalary(currentCNP, year, month).equals("Date inexistente")) {
					if (!app.getFunctionUser(currentCNP).equals("Medic")) {
						salaryTotal += new Double(app.getSalary(currentCNP, year, month));
					} else {
	...
	private void initializeSalary() throws SQLException {
		...
		while (rstCNP.next()) {
			...
			ResultSet rst = app.getSalaryAndHoursPerMonth(currentCNP);
			...
			LocalDate date = LocalDate.parse(app.getHiringDate(currentCNP));
			int indexMonth = date.getMonthValue();
			int indexYear = date.getYear();
			LocalDate currentDate = LocalDate.now();
			int days = 0;

			rstGenericSchedule = app.getGenericSchedule(currentCNP, RO.convertToRO(date.getDayOfWeek().toString()));
			...
			if (rstGenericSchedule.next() && (rstGenericSchedule.getString(2) != null)) {
				while (date.getMonthValue() < currentDate.getMonthValue() || date.getYear() < currentDate.getYear()) {
					...
					if (!app.checkHistoryOfTheSalary(currentCNP, date.getMonthValue(), date.getYear())) {
						rstGenericSchedule = app.getGenericSchedule(currentCNP,
						...
						if (indexMonth != date.getMonthValue()) {
							...
							if (app.getFunctionUser(currentCNP).equals("Medic")) {
								...
								ResultSet rstBill = app.getBillSpecificDate(currentYear, indexMonth);
								while (rstBill.next()) {
							...
							app.insertHistoryOfTheSalary(currentCNP, indexMonth, currentYear, salaryPerMounth);
							days = 0;
	...
	private void setSalaryText() throws SQLException {
		...
		if (!yearSalaryComboBox.getSelectionModel().isEmpty() && !monthSalaryComboBox.getSelectionModel().isEmpty()) {
			txtSalary.setText(app.getSalary(logInCNP, yearSalaryComboBox.getSelectionModel().getSelectedItem(),
	...
	private boolean checkVacation(String CNP, String dateCheck) throws SQLException {
		...
		ResultSet rst = app.getVacation(CNP);
	...
	private void initializeUserSearchTableView() throws SQLException {
		...
		ResultSet rst = app.getUserDataForSearch(txtNume.getText(), txtPrenume.getText(), txtFunction.getText());

		while (rst.next()) {
			if (!rst.getString(1).equals(app.getCNPUser(Main.getUserName())))
				userSearchList
