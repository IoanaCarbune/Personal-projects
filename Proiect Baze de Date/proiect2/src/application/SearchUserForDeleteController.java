...
public class SearchUserForDeleteController {
	...
	private void showSeaerchRezult() throws SQLException {
		...
		ResultSet rst = app.getUserDataForSearch(txtNume.getText(), txtPrenume.getText(), txtFunctia.getText());
	...
	private void deleteButton() throws SQLException {
		...
		if (app.getTypeUser(Main.getUserName()).equals("Super-administrator")) {
			app.deleteUser(CNP);
		} else if (app.getTypeUser(app.getUserName(CNP)).equals("Angajat")) {
			app.deleteUser(CNP);
		}
