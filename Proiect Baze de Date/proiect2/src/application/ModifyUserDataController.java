...
public class ModifyUserDataController {
	...
	private void initialize() throws SQLException {
		...
		specialtiesList = app.getSpecialties();
		...
		if (!app.getTypeUser(userName).equals("Super-administrator")) {
			typeList.remove(2);
			typeList.remove(1);
		}
		...
		if (Main.isAccountDate()) {
			...
			if (!(app.getTypeUser(Main.getUserName()).equals("Administrator")
	...
	private void showUserData() throws SQLException {
		...
		if (!userSearchTableView.getSelectionModel().isEmpty() || Main.isAccountDate()) {
			...
			if (Main.isAccountDate())
				currentCNP = app.getCNPUser(Main.getUserName());
			else {

				currentCNP = userSearchTableView.getSelectionModel().getSelectedItem().getUserCNP();
			}

			String function = app.getFunctionUser(currentCNP);
			System.out.println(currentCNP + "   " + function);
			data = app.getAllUserData(currentCNP, function);
			...
			password1.setText(app.decryptPassword(app.getUserName(currentCNP)));
			password2.setText(app.decryptPassword(app.getUserName(currentCNP)));
			...
			if (function.equals("Asistent medical")) {
				...
				List<Integer> medicSkills = app.getMedicSkills(currentCNP);
				List<Integer> medicSpecialties = app.getMedicSpecialties(currentCNP);
...

	{
		...
		ResultSet rst = app.getUserDataForSearch(txtNumeForSearch.getText(), txtPrenumeForSearch.getText(),
				txtFunctionForSearch.getText());

		while (rst.next()) {
			if (app.getTypeUser(Main.getUserName()).equals("Super-administrator")) {

				userSearchList
						.add(new UserSearch(rst.getString(1), rst.getString(2), rst.getString(3), rst.getString(4)));

			} else if (app.getTypeUser(Main.getUserName()).equals("Administrator")) {
	...
	private void buttonOk() throws SQLException, IOException, InterruptedException {
		...
		if (txtNume.getLength() == 0 || txtPrenume.getLength() == 0 || typeUser.getValue() == null
		...
		} else if (!txtCNP.getText().toString().equals(currentCNP) && app.checkCNP(txtCNP.getText())) {
			displayMessage("Exista acest CNP");
		} else if (!txtUsername.getText().toString().equals(app.getUserName(currentCNP))
				&& (txtUsername.getText().toString().length() != 0
						&& app.checUserName(txtUsername.getText().toString()))) {
			...
			if (Main.isAccountDate() || app.getUserName(currentCNP).equals(Main.getUserName())) {
				Main.setUserName(txtUsername.getText());

			}

			app.setForeignKeyChecks(0);
			app.updateUser(currentCNP, txtCNP.getText(), txtNume.getText(), txtPrenume.getText(), txtUsername.getText(),
			...
			if (functions.getValue() != null) {
				...
				if (functions.getValue().equals("Medic")) {
					...
					if (function.equals("Medic"))
						app.updateMedicData(currentCNP, txtCNP.getText(), grad, txtCodParafa.getText(),
								txtTitluStiintfic.getText(), txtPostDidactic.getText(), txtProcentNegociat.getText());
					else
						app.insertMedicData(txtCNP.getText(), grad, txtCodParafa.getText(), txtTitluStiintfic.getText(),
								txtPostDidactic.getText(), txtProcentNegociat.getText());

					int i = 1;
					for (CheckBox element : checkboxesSpecialties) {
						if (element.isSelected()) {
							app.insertMedicSpecialties(txtCNP.getText(), i);
						}
						i++;
					}
					...
					for (CheckBox element : checkboxesSkills) {
						if (element.isSelected()) {
							app.insertMedicSkills(txtCNP.getText(), i);
						}
						i++;
					}
					...
					if (function.equals("Asistent medical"))
						app.updateAsistent(currentCNP, txtCNP.getText(), tip, grad);
					else
						app.insertAsistent(txtCNP.getText(), tip, grad);
				} else {
					setMedicNULL();
					setAsistentNULL();
				}
