...
public class AppointmentController {
	...
	private void initialize() throws SQLException, ParseException {
		...
		service.setItems(app.getAllServices());
		serviceSearch.setItems(app.getAllServices());
	...
	private void searchServices(KeyEvent event) throws SQLException {
		...
		System.out.println(app.getSpecificService(text));
		service.getSelectionModel().select(app.getSpecificService(text));
		service.setValue(app.getSpecificService(text));
	...
	private void searchAppointment() throws SQLException {
		...
		ResultSet rst = app.getMedicForSpecificService(currentService);
		...
		while (rst.next()) {
			...
			ResultSet specificTimeTable = app.getSpecificTimeTable(currentCNP, date);
			ResultSet genericTimeTable = app.getGenericSchedule(currentCNP, dayOfWeek);
			...
			if (!checkVacation(currentCNP, date))
				...
				currentMedicalUnit = app.getMedicalUnitForSpecificDay(currentCNP, date, "");
				...
				currentMedicalUnit = app.getMedicalUnitForSpecificDay(currentCNP, "", dayOfWeek);
			}
			...
			for (calendar.setTime(start); calendar.getTime().before(end); calendar.add(Calendar.MINUTE, 5)) {
				...
				aux.add(Calendar.MINUTE, app.getServiceTime(currentService));
	...
	private boolean checkIntervals(Time start, Time end, String date, String CNP) throws SQLException {
		...
		ResultSet rst = app.getAllApointments(CNP, date);
		while (rst.next()) {
	...
	private void addAppointment() throws SQLException {
		...
		if (txtPatientFitrstName.getText().toString().length() == 0
			...
			calendar.add(Calendar.MINUTE, app.getServiceTime(service.getSelectionModel().getSelectedItem().toString()));
			...
			String serviceId = app.getServiceId(service.getSelectionModel().getSelectedItem().toString());

			app.insertAppointment(txtPatientLastName.getText(), txtPatientFitrstName.getText(), start, end,
	...
	private void initializeAppointmentTableView() throws SQLException {
		...
		ResultSet rst = app.getAllAppointmentsData(txtDoctorLastName.getText(), txtDoctorFirstName.getText(), "",
	...
	private void deleteAppointment() throws SQLException {
		app.deleteAppointment(appointmentsTableView.getSelectionModel().getSelectedItem().getCNP(),
