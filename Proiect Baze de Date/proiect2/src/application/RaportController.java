...
public class RaportController {
	...
	private DataBase app = new DataBase();
	...
	private void initialize() throws SQLException {
		...
		if (app.getFunctionUser(app.getCNPUser(Main.getUserName())).equals("Medic")
				|| app.getTypeUser(Main.getUserName()).equals("Administrator")
				|| app.getTypeUser(Main.getUserName()).equals("Super-administrator")) {
			titleRaport.setText("Raport medical");
			typeRaport = "RM";
			initializeUnfinishedReportTableView();

			ResultSet rst = app.getMedicSpecialitiesNames(app.getCNPUser(Main.getUserName()));
		...
		} else if (app.getFunctionUser(app.getCNPUser(Main.getUserName())).equals("Asistent medical")) {
			titleRaport.setText("Raport analize");
			services.setItems(app.getFilteredServices("asistent medical"));
			typeRaport = "RA";
			initializeUnfinishedReportTableView();
	...
	public void initializeServicesComboBox() throws SQLException {
		services.setItems(app.getServicesFilteredBySpeciality(
	...
	void initializeUnfinishedReportTableView() throws SQLException {
		...
		ResultSet rst = app.getUnfinishedRaports(app.getCNPUser(Main.getUserName()), typeRaport,
	...
	private void initializePatientName() throws SQLException {
		ResultSet rst = app.getPatientName(txtPatientCNP.getText());
		if (txtPatientCNP.getLength() == 13 && !app.checkPatientCNP(txtPatientCNP.getText())) {
	...
	private void saveButton() throws SQLException {
		...
		ResultSet rst = app.getCurrentUserName(app.getCNPUser(Main.getUserName()));
		if (rst.next()) {
		...
		if (txtPatientCNP.getLength() == 0) {
		...
		} else if (!app.checkPatientCNP(txtPatientCNP.getText())) {
			MessageController.displayMessageLabel(errorLabel, "pacient neinregistrat", Color.RED);
			...
			app.insertRaport(txtPatientCNP.getText(), medicLastName, medicFirstName, app.getCNPUser(Main.getUserName()),
			...
			for (Service element : servicesList) {
				app.insertServiceRaport(app.getLastidRaport(), app.getServiceId(element.getInvestigation()),
						element.getResult().getText());
			}
			newRaport = false;
			initializeUnfinishedReportTableView();
			currentIdRaport = app.getLastidRaport();
			MessageController.displayMessageLabel(errorLabel, "Raport salvat cu succes", Color.GREEN);
		} else {
			app.updateRaport(currentIdRaport, txtPatientCNP.getText(), medicLastName, medicFirstName,
					app.getCNPUser(Main.getUserName()), txtRecDoctorLastName.getText(), txtRecDoctorFirstName.getText(),
					txtNurseLastName.getText(), txtNurseFirstName.getText(), txtSimptoms.getText(),
					txtDiagnostic.getText(), txtRecommendation.getText(), "", typeRaport);

			ResultSet rstServices = app.getReportServices(currentIdRaport);
			...
			while (rstServices.next()) {
				...
				for (Service element : servicesList) {
					if (element.getInvestigation().equals(currentService)) {
						app.updateResultServiceFromRaport(currentIdRaport, app.getServiceId(currentService),
								element.getResult().getText());
						ok = 1;
					}
				}
				if (ok == 0) {
					...
					app.deleteServiceFromRaport(currentIdRaport, app.getServiceId(currentService));
				}
			}

			for (Service element : servicesList) {
				rstServices = app.getReportServices(currentIdRaport);
				int ok = 1;
				while (rstServices.next()) {
					String currentService = rstServices.getString(1);
					if (element.getInvestigation().equals(currentService)) {
				...
				if (ok == 1) {
					app.insertServiceRaport(currentIdRaport, app.getServiceId(element.getInvestigation()),
							element.getResult().getText());
				}
			}
	...
	private void submitButton() throws SQLException {
		...
		if (txtPatientCNP.getLength() == 0) {
			...
			MessageController.displayMessageLabel(errorLabel, "Puneti parafa", Color.RED);
				...
				&& !txtSeal.getText().equals(app.getMedicSeal(app.getCNPUser(Main.getUserName())))) {
			...
			initializeUnfinishedReportTableView();
			currentIdRaport = app.getLastidRaport();
			app.setRaportFinished(currentIdRaport);
	...
	private void deleteButton() throws SQLException {
		...
		if (!unfinishedReportTableView.getSelectionModel().isEmpty()) {
			app.deleteUnfinishedRaport(unfinishedReportTableView.getSelectionModel().getSelectedItem().getIdRaport());
			initializeUnfinishedReportTableView();
			newRaportButton();
		}
	}
