...
public class BillController {
	...
	private void initializeTableView() throws SQLException {
		...
		ResultSet rst = app.getPacientRaport(txtPatientCNP.getText(), txtPatientLastName.getText(),
				txtPatientFirstName.getText());
		while (rst.next()) {

			int currentRaportId = rst.getInt(1);
			ResultSet currentService = app.getReportServices(currentRaportId);
	...
	private void initializeBillArea() throws SQLException {
		...
		if (!unpaidServicesTable.getSelectionModel().isEmpty()) {
			...
			ResultSet currentUser = app.getCurrentUserName(app.getCNPUser(Main.getUserName()));
			String currentUserName = null;
			while (currentUser.next()) {
				currentUserName = currentUser.getString(1) + " " + currentUser.getString(2);
			}

			ResultSet rstRaportData = app.getAllRaportsData(currentRaportId);
			if (rstRaportData.next()) {
				...
				if (currentRaportType.equals("RM")) {
					...
					medicData = app.getAllUserData(currentRaportMedicCNP, "Medic");
					...
					ResultSet rst = app.getReportServices(currentRaportId);
					while (rst.next()) {
						...
						if (rst.getString(3) == null) {
							...
							// app.payService(currentRaportId, rst.getString(5));
	...
	private void printButton() throws SQLException {
		...
		if (!unpaidServicesTable.getSelectionModel().isEmpty()) {
			...
			ResultSet specificTimeTable = app.getSpecificTimeTable(currentAuthorCNP, date);

			String medicalUnit = new String();

			if (specificTimeTable.next())
				medicalUnit = app.getMedicalUnitForSpecificDay(currentAuthorCNP, date, "");
			else
				medicalUnit = app.getMedicalUnitForSpecificDay(currentAuthorCNP, "", dayOfWeek);

			app.processBill(currentAuthorCNP, total,LocalDate.now().toString(), medicalUnit, speciality);
			initializeTableView();
			initializeBillArea();
		}
	}
