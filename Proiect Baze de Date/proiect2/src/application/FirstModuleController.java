...
public class FirstModuleController {
	...
	private void initialize() throws SQLException {
		...
		String userCNP = app.getCNPUser(userName);
		String function = app.getFunctionUser(userCNP);
		String userType = app.getTypeUser(userName);
	...
	private void initializeGenericTimeTable(String CNP) throws SQLException {
		...
		ResultSet rst = app.getGenericSchedule(CNP, "");
		while (rst.next()) {
	...
	private void initializeSpecificTimeTable(String CNP) throws SQLException {
		...
		ResultSet rst2 = app.getSpecificTimeTable(CNP, "");
	...
	private void addVacationButton() throws SQLException {
		...
		if (app.checkGenericSchedule(currentCNP)) {
			...
			app.insertVacation(currentCNP, dateStartVacation.getValue().toString(),
					dateEndtVacation.getValue().toString());
			initializeVacation(currentCNP);
		}
	}
	...
	private void deleteVacationButton() throws SQLException {
		...
		app.deleteVacation(currentCNP, element.getStart());
		initializeVacation(currentCNP);
	}
	...
	private void addSpecificButton() throws SQLException {
		...
		String currenFunction = app.getFunctionUser(currentCNP);
		...
		if (app.checkGenericSchedule(currentCNP)) {
		...
		} else if (!app.checkMedicalUnit(txtMedicalUnintSpecific.getText().toString())
			...
			app.insertSpecificTimeTable(currentCNP, dateSpecific.getValue().toString(), txtStartSpecific.getText(),
	...
	void deleteSpecificButton() throws SQLException {
		...
		app.deleteSpecificTimeTable(currentCNP, element.getDay());
		initializeSpecificTimeTable(currentCNP);
	}
	...
	private void modifySpecificButton() throws SQLException {
		...
		for (TimeTable element : specificTimeTableList) {
			...
			if (!app.checkMedicalUnit(element.getMedicalUnit()) || element.getMedicalUnit().equals("0")) {
		...
		if (ok == 1) {
			...
			for (TimeTable element : specificTimeTableList) {
				...
				app.updateSpecificTimeTable(CNP, element.getDay(), element.getStart(), element.getEnd(),
	...
	private boolean checkTimeIntervals(String MU, String day, Time start, Time end) throws SQLException {
		...
		ResultSet rst = app.getMedicalUnitTimeTable(MU, day);
	...
	private void modifyButton() throws SQLException {
		...
		for (TimeTable element : genericTimeTableList) {
			...
			if (!app.checkMedicalUnit(element.getMedicalUnit())) {
		...
		if (ok == 1) {
			...
			for (TimeTable element : genericTimeTableList)
				app.updateGenericTimeTable(CNP, element.getDay(), element.getStart(), element.getEnd(),
						element.getMedicalUnit());
		}
	}
